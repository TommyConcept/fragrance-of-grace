...
export default function FeaturedVideos() {
  return (
    <section
      id="videos"
      className="bg-navy py-20 md:py-28 lg:py-32"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-14 md:mb-18">
          ...
          <p className="text-ivory/60 font-inter text-base max-w-xl mx-auto leading-relaxed">
        ...
        >
          {featuredVideos.map((video) => (
            <article
              key={video.id}
              className="group bg-white/5 rounded-2xl overflow-hidden border border-white/10 hover:border-gold/30 hover:bg-white/8 transition-all duration-300"
              role="listitem"
            >
              {/* Thumbnail */}
              <div className="relative overflow-hidden aspect-video bg-navy/50">
                <img
                  src={video.thumbnail}
                ...
                {/* Dark overlay */}
                <div
                  className="absolute inset-0 bg-navy/30 group-hover:bg-navy/15 transition-colors duration-300"
                  aria-hidden="true"
                />
                ...
                <div className="absolute top-3 right-3">
                  <span className="inline-flex px-2 py-1 rounded bg-gold/80 text-white text-xs font-inter font-medium">
                    {video.label}
                  </span>
                </div>
              </div>
              ...
              <div className="p-5 flex flex-col gap-3">
                <h3
                  className="font-playfair text-ivory text-base font-semibold leading-snug group-hover:text-gold transition-colors"
                  style={{ fontFamily: "'Playfair Display', Georgia, serif" }}
                >
                  {video.title}
                </h3>
                <p className="text-ivory/55 font-inter text-xs leading-relaxed line-clamp-2">
                  {video.description}
                </p>
                <a
                  href={video.link}
                  ...
                  className="mt-auto inline-flex items-center gap-2 text-gold font-inter text-xs font-medium hover:text-gold-light transition-colors group/btn"
