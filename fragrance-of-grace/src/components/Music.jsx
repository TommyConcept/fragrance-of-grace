...
// Worship songs: Your Will Be Done, You Alone Are Worthy, Sovereign God
...
function MusicCard({ song }) {
  return (
    <article className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gold/10 hover:shadow-md hover:border-gold/25 transition-all duration-300">
      {/* Album art — full area links to YouTube */}
      <a
        href={song.youtubeLink}
        target="_blank"
        rel="noopener noreferrer"
        className="relative block aspect-video bg-navy/90 overflow-hidden group"
      ...
      >
        ...
        />
        {/* Dark overlay */}
        <div className="absolute inset-0 bg-navy/50 group-hover:bg-navy/25 transition-colors" aria-hidden="true" />

        {/* Play button — centered */}
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-16 h-16 rounded-full bg-gold group-hover:scale-110 transition-transform flex items-center justify-center shadow-lg">
            <Youtube size={26} className="text-white ml-1" aria-hidden="true" />
          </div>
        </div>

        {/* Label */}
        <p className="absolute bottom-3 left-3 text-ivory/80 font-inter text-xs tracking-wide">
          Watch on YouTube
        </p>
      </a>
      ...
      <div className="p-6 sm:p-7">
        ...
        <div className="mt-4 pt-4 border-t border-gray-100">
          <p className="text-muted font-inter text-xs tracking-wide mb-2">Also available on:</p>
          <div className="flex flex-wrap gap-x-3 gap-y-1.5">
            {['Apple Music', 'Amazon Music', 'Pandora', 'TikTok', 'Rumble'].map((platform) => (
              <span
                key={platform}
                className="text-muted font-inter text-xs px-2.5 py-1 rounded-full bg-gray-50 border border-gray-100"
              >
                {platform}
              </span>
            ))}
